...
TPUART2SerialLayer2Driver::TPUART2SerialLayer2Driver (const char *dev,
...
TPUART2SerialLayer2Driver::~TPUART2SerialLayer2Driver ()
{
...
bool TPUART2SerialLayer2Driver::init ()
{
  return fd != -1;
}

bool
TPUART2SerialLayer2Driver::addAddress (eibaddr_t addr)
{
  unsigned i;
  for (i = 0; i < indaddr (); i++)
    if (indaddr[i] == addr)
...
bool
TPUART2SerialLayer2Driver::addGroupAddress (eibaddr_t addr)
{
  unsigned i;
...
bool
TPUART2SerialLayer2Driver::removeAddress (eibaddr_t addr)
{
  unsigned i;
  for (i = 0; i < indaddr (); i++)
    if (indaddr[i] == addr)
...
bool
TPUART2SerialLayer2Driver::removeGroupAddress (eibaddr_t addr)
{
  unsigned i;
  for (i = 0; i < groupaddr (); i++)
    if (groupaddr[i] == addr)
...
bool TPUART2SerialLayer2Driver::openVBusmonitor ()
{
  vmode = 1;
  return 1;
}

bool TPUART2SerialLayer2Driver::closeVBusmonitor ()
{
  vmode = 0;
  return 1;
}
...
eibaddr_t
TPUART2SerialLayer2Driver::getDefaultAddr ()
{
  return addr;
}

bool
TPUART2SerialLayer2Driver::Connection_Lost ()
{
  return 0;
}

bool
TPUART2SerialLayer2Driver::Send_Queue_Empty ()
{
  return inqueue.isempty ();
}

void
TPUART2SerialLayer2Driver::Send_L_Data (LPDU * l)
{
...
LPDU *
TPUART2SerialLayer2Driver::Get_L_Data (pth_event_t stop)
{
  if (stop != NULL)
...
bool
TPUART2SerialLayer2Driver::enterBusmonitor ()
{
  uchar c = 0x05;
...
bool
TPUART2SerialLayer2Driver::leaveBusmonitor ()
{
  uchar c = 0x01;
...
bool
TPUART2SerialLayer2Driver::Open ()
{
  uchar c = 0x01;
...
bool
TPUART2SerialLayer2Driver::Close ()
{
  return 1;
}

void
TPUART2SerialLayer2Driver::RecvLPDU (const uchar * data, int len)
{
...
void
TPUART2SerialLayer2Driver::Run (pth_sem_t * stop1)
{
  uchar buf[255];
  int i;
  CArray in;
